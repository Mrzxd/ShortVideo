...
- (instancetype)init {
    ...
    if(self) {
        ...
        _memCache.totalCostLimit = 600 * 1024 * 1024;
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
            if (weakSelf.memCache) {
                [weakSelf.memCache removeAllObjects];
            }
        }];
        //初始化文件管理类
...
natural_t usedMemory() {
    ...
    if (kerr == KERN_SUCCESS) {
        return info.resident_size / (1024 * 1024);// 返回MB单位
    } else {
        return 0;
    }
}

// 向NSCache中添加数据，仅当已使用内存小于某个阈值时
void addObjectToCacheSafely(NSCache *cache, NSString *key, id object) {
    const natural_t memoryLimit = 600; // 设置内存上限为600MB
    pthread_mutex_lock(&WebCacheHelpler.sharedWebCache->_lock);
    
    natural_t memoryUsed = usedMemory();
    if (memoryUsed > memoryLimit) {
        [cache removeAllObjects];
    }
    pthread_mutex_unlock(&WebCacheHelpler.sharedWebCache->_lock);
    
    if (pthread_mutex_trylock(&WebCacheHelpler.sharedWebCache->_lock) == 0) {
        natural_t memoryUsed = usedMemory();
        if (memoryUsed < memoryLimit) {
            [cache setObject:object forKey:key];
        } else {
            // 已使用内存超过限制，可以执行一些处理逻辑，如移除部分对象或通知用户等
            [cache removeAllObjects];
            [cache setObject:object forKey:key];
    //        NSLog(@"Memory usage exceeded the limit of %luMB. Current usage: %luMB", (unsigned long)memoryLimit, (unsigned long)memoryUsed);
        }
        pthread_mutex_unlock(&WebCacheHelpler.sharedWebCache->_lock);
    }
}
...
- (void)storeDataToMemoryCache:(NSData *)data key:(NSString *)key {
    if(data && key) {
        addObjectToCacheSafely(self.memCache, key, data);
    }
}
//存储缓存数据到本地磁盘
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    ...
    if (self.progressBlock) {
        self.progressBlock(_dataLength, self.expectedSize, data);
    }
}
...
- (instancetype)init {
    ...
    if(self) {
        ...
        _downloadPriorityHighQueue.maxConcurrentOperationCount = 6;
...
//    if (range.location > 0 && range.length > 0) {
        NSString *value = [NSString stringWithFormat:@"bytes=%lld-", range.location];
